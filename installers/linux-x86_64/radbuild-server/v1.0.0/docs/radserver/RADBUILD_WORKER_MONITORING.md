...
```bash
nohup <radbuild-install-prefix>/radbuildserver \
  --config <radbuild-install-prefix>/radserver_data/radbuildserver.json \
  > <radbuild-install-prefix>/radserver_data/radbuildserver.nohup.log 2>&1 &
```
...
`build_vivado` and `build_petalinux` command names where possible.
...
[Service]
WorkingDirectory=<radbuild-install-prefix>
ExecStart=<radbuild-install-prefix>/radserver serve --host 0.0.0.0 --port 8767
Restart=always
RestartSec=10
