...
```bash
radbuildserver --config /home/jvincent/RadBuild/radserver_data/radbuildserver.json
```
...
steps from a sourced RadBuild shell. Build steps should use `build_vivado`,
`build_litex`, and `build_petalinux` command names where possible.

## Discord Notifications

Discord bot-token notifications are configured from runtime JSON, not from the
packaged RadTools release. Keep the real token in the installed server data
directory or point `RADBUILD_DISCORD_CONFIG` at another private file:

```text
/home/jvincent/RadTools/RadBuild/v1.0.0/radserver_data/discord_config.json
```
...
```json
{
  ...
  "channel_name": "radbuildserver",
  "guild_id": "",
  "channel_id": ""
}
```

Use `channel_id` when the bot is in multiple servers with the same channel
name. The release template only contains empty placeholders, and local Discord
config files are ignored by git.

Send a test message:

```bash
radserver test-discord --message "RadBuild server Discord notifications are online."
```
